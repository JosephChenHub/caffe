...
//  const int label_size = this->layer_param_.multi_label_data_param().label_size();
  ...
  while (std::getline(infile, line)) {
    boost::split(splits, line, boost::is_any_of(",")); //split by comma
    labels.clear();
    for(size_t i = 1; i < splits.size(); ++i) {
        label = atof(splits[i].c_str());
        labels.push_back(label);
    }
    lines_.push_back(std::make_pair(splits[0], labels));
  }
  ...
  if (this->layer_param_.multi_label_data_param().shuffle()) {
    ...
    if (this->phase_ == TRAIN && Caffe::solver_rank() > 0 &&
        this->layer_param_.multi_label_data_param().rand_skip() == 0) {
  ...
  LOG(INFO) << "output data size: " << top[0]->num() << ","
      ...
      << top[0]->width();
  // labels  NxCx1x1
  top[1]->Reshape(batch_size, labels.size(),1, 1);
  for (int i = 0; i < this->prefetch_.size(); ++i) {
    this->prefetch_[i]->label_.Reshape(batch_size, labels.size(), 1, 1);
  }
}
...
void MultiLabelDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    for(size_t i = 0; i < labels.size(); ++i) {
        prefetch_label[offset2 + i] = labels[i];
    } 
