...
#else
    caffe_copy(N*c_, Y_data, bottom_C_.mutable_gpu_data());
    caffe_gpu_vm_mul(N, bottom_C_alpha_.gpu_data(), c_, bottom_C_.mutable_gpu_data());
    caffe_gpu_vm_add(N, bottom_C_beta_.gpu_data(), c_, bottom_C_.mutable_gpu_data());
#endif    
    /// A = (D*Y)1
...
#if 1
    caffe_gpu_mul(N, bottom_A_.gpu_data(), bottom_inv_mul_.gpu_data(), bottom_A_.mutable_gpu_data());
#endif    
...
#if 1
            caffe_gpu_vm_mul(N, bottom_inv_mul_.gpu_data(), c_, bottom_Y1_.mutable_gpu_data());
            caffe_gpu_vm_mul(N, bottom_inv_mul_.gpu_data(), c_, bottom_Y_.mutable_gpu_data());
            Dtype beta = 2.0/N*2;
#else   
